...
const App: React.FC = () => {
  ...
  const handleSendMessage = async (content: string) => {
    ...
    try {
      ...
      setIsGenerating(false)
      fetchConversations() // Refresh thread list to update sorting/title update
    }
  }
          ...
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSaveSettings}>
              <div className="modal-body">
                ...
                {/* Primary LLM Model */}
                <div className="form-group">
                  <label className="form-label">Generation Model (Ollama)</label>
                  {models.length === 0 ? (
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', background: 'rgba(255,255,255,0.02)', padding: '0.75rem', borderRadius: '8px', border: '1px solid var(--border-light)' }}>
                      No local models detected in Ollama. Make sure Ollama has pulled a model (e.g. <code>ollama pull llama3.2</code>) and is running.
                      <input 
                        type="text" 
                        ...
                        placeholder="Fallback model name (e.g. llama3.2)"
                      />
                    </div>
                  ) : (
                    <select
                    ...
                    >
                      {models.map((m) => (
                        <option key={m.name} value={m.name}>
                          {m.name} ({(m.size / 1e9).toFixed(2)} GB)
                        </option>
                      ))}
                    </select>
                  )}
